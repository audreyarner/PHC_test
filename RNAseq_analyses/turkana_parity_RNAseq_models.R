...
#Take only those individs who are strictly pastoralist or urban
control_data <- control_data %>% filter(Lifestyle!="RuralNotPastoral")

control_data <- control_data %>% filter(Lifestyle=="Pastoralist")

control_data <- control_data %>% filter(Gender == "F")
...
pretty_up_cols=function(cols){
  ...
  cols=mean_center_clean(cols,"NumberPreg")
...
emma_parity <- data.frame(t(parApply(clus, resids, 1, function(y){
  library(EMMREML)
  emma <- emmreml(y = y,  # model each gene
                  X = model.matrix(~MergeAll2$Age + MergeAll2$NumberPreg + MergeAll2$Count + MergeAll2$PC1),  # design matrix
...
LifestyleSexFDR <- unname(unlist(emma_parity[,"V13"]))
fdr <- p.adjust(LifestyleSexFDR, method = "fdr")
emma_parity$pdiff.FDR <- fdr

qq(emma_parity$V13)

hist(emma_parity$V13)

sum(emma_parity$pdiff.FDR < 0.1)

write.table(emma_parity, "TurkanaPastoralistParityLMMOutput.txt", quote=F, sep = "\t")
...
sum(emma_parity$pdiff.FDR < 0.05)

write.table(emma_parity, "TurkanaUrbanParityLMMOutput.txt", quote=F, sep = "\t")
...
for(j in 1:10){
  ...
  MergeAll2$PregPermute <- sample(MergeAll2$NumberPreg, nrow(MergeAll2), replace = FALSE, prob = NULL)
  
  emma_parity <- data.frame(t(parApply(clus, resids, 1, function(y){
    library(EMMREML)
    emma <- emmreml(y = y,  # model each gene
                    X = model.matrix(~MergeAll2$Age + MergeAll2$PregPermute + MergeAll2$Count + MergeAll2$PC1 + MergeAll2$Lifestyle),  # design matrix
                    Z = as.matrix(mat),  # identity matrix
                    K = as.matrix(GRM3),  # relatedness matrix
                    varbetahat = T, varuhat = T, PEVuhat = T, test = T)
    p=emma$pvalbeta
  ...
  PTable <- emma_parity %>% dplyr::select(V15) 
...
FigOverall <- ggplot(GeomPolyTable, aes(V21, color=Permutation)) +
  geom_freqpoly(bins=100)

ggplot(GeomPolyTable, aes(V21)) +
  geom_freqpoly(bins=100)

write.table(GeomPolyTable, "TurkanaParityLMMEmpiricalFDR.txt", quote=F, sep = "\t")
