...
#p-values and effects as calculated from limma/voom
results=read.delim('TwaKigaRawDEAnalysis1-31-22.txt', sep=" ")

#p-value permutations
GeomPolyTable <- read.delim("TwaKigaSexLMempiricalPerm.txt")

#create table in format required for function
PermutationTable<-matrix(ncol=20,nrow= dim(results)[1])
for (i in 1:20){
  PermutationTable[,i]<-subset(GeomPolyTable,Permutation==i)$P.Value
}

DEGenes<-perm.fdr(results, PermutationTable, "P.Value")

#identify significant genes at cutoff
length(which(DEGenes$q_ST_perm<0.05))
length(which(DEGenes$q_ST_perm<0.1))

qq(GeomPolyTable$P.Value)

write.table(DEGenes, "TwaKigaDEGenesEmpiricalFDR1-31-23.txt", quote=F)
