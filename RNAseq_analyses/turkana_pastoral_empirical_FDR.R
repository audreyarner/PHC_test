...
#permutation code from Snyder-Mackler
...
#raw results
results=read.delim('TurkanaLifestyleLMEOutput.txt')

#p-value permutations
GeomPolyTable1=read.delim("TurkanaLifestyleLMMempiricalPerm.txt")

#make permutation table, where rows are genes and columns are each permutation
PermutationTable<-matrix(ncol=5,nrow= dim(results)[1])
for (i in 1:5){
  PermutationTable[,i]<-subset(GeomPolyTable1,Permutation==i)$V21
}

#run empirical FDR perm
DEGenes<-perm.fdr(results, PermutationTable, "V21")

length(which(DEGenes$q_ST_perm<0.05))

length(which(DEGenes$q_ST_perm<0.1))

write.table(DEGenes, "TurkanaDEGenesEmpFDR.txt", quote=F)
